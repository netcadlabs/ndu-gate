...
# from ..ssd.config import fd_config
from ndu_gate_camera.detectors_old_sil.vision.nn.mb_tiny import Mb_Tiny
...
def create_mb_tiny_fd(num_classes, is_test=False, device="cuda"):
    ...
    return SSD(num_classes, base_net_model, source_layer_indexes,
               extras, classification_headers, regression_headers, is_test=is_test, config=fd_config, device=device)


def create_mb_tiny_fd_predictor(net, candidate_size=200, nms_method=None, sigma=0.5, device=None):
    predictor = Predictor(net, fd_config.image_size, fd_config.image_mean_test,
                          fd_config.image_std,
                          nms_method=nms_method,
                          iou_threshold=fd_config.iou_threshold,
