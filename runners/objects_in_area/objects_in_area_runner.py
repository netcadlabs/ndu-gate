...
class objects_in_area_runner(Thread, NDUCameraRunner):
    ...
    def process_frame(self, frame, extra_data=None):
        ...
        if len(self.__areas) == 0 and self.__frame_num == 1:
            areas = image_helper.select_areas(frame, self.get_name(), color=self.__debug_color_normal, opacity=0.3, thickness=self.__debug_thickness_normal, max_count=None, next_area_key="n", finish_key="s")
            # areas = [[(503, 167), (403, 225), (565, 270), (639, 203)], [(334, 484), (307, 483), (277, 481), (229, 491), (210, 518), (196, 546), (213, 580), (230, 605), (280, 626), (309, 627), (350, 615), (374, 598), (405, 567), (411, 532), (384, 501)]] #vid_short.mp4
            ...
            for area in areas:
                area_counter += 1
                self.__areas.append({"name": f"ALAN{area_counter}", "area": area})
        ...
        for item in self.__areas:
            ...
            if self.__debug:
                ...
                image_helper.put_text(frame, name, center, color=self.__debug_color_normal, font_scale=0.75)
        ...
        if results is not None:
                    ...
                    if class_name in self.__classes:
                        ...
                        if rect is not None:
                            for area_item in self.__areas:
                                ...
                                if geometry_helper.is_inside(area, ref_pnt):
                                    # if geometry_helper.is_inside(area, (ref_pnt[1], ref_pnt[0])):
                                    ...
                                    if self.__debug: #and self.__frame_num % 2 == 0:
                                        ...
                                        center = self.get_center_int(area)
                                        image_helper.put_text(frame, name, center, color=self.__debug_color_alert, font_scale=0.75)
            ...
            for area_name, item in counts.items():
                ...
                if self.__debug:
                    debug_text = f"{area_name}: "
                for class_name, value in counts.get(area_name).items():
                    if self.__debug:
                        debug_text += f"{class_name}: {value} "
        ...
        for item in self.__areas:
            ...
            if name in active_areas:
                ...
                data_txt += name + "_0"
        if True:#self.__last_data != data_txt:
